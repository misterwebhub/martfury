<div class="image-box">
    ...
    <div class="preview-image-wrapper @if (!Arr::get($attributes, 'allow_thumb', true)) preview-image-wrapper-not-allow-thumb @endif">
        <img src="{{ RvMedia::getImageUrl($value, Arr::get($attributes, 'allow_thumb', true) == true ? 'thumb' : null, false, RvMedia::getDefaultImage()) }}" data-default="{{ RvMedia::getImageUrl(RvMedia::getDefaultImage(), 'thumb') }}" alt="{{ trans('core/base::base.preview_image') }}" class="preview_image" @if (Arr::get($attributes, 'allow_thumb', true)) width="150" @endif>
