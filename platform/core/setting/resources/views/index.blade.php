...
@section('content')
    <div id="main-settings">
        <license-component verify-url="{{ route('settings.license.verify') }}"
            activate-license-url="{{ route('settings.license.activate') }}"
            deactivate-license-url="{{ route('settings.license.deactivate') }}"
            reset-license-url="{{ route('settings.license.reset') }}"></license-component>
    </div>
    ...
    <div class="max-width-1200">
        <div class="flexbox-annotated-section">
            ...
            <div class="flexbox-annotated-section-content">
                <div class="wrapper-content pd-all-20">
                    ...
                    <div class="form-group mb-3" id="admin_email_wrapper"
                        data-emails="{{ json_encode(get_admin_email()) }}" data-max="{{ $maxEmailCount }}">
                        <label class="text-title-field"
                            for="admin_email">{{ trans('core/setting::setting.general.admin_email') }}</label>
                        <a id="add" class="link" data-placeholder="email{{ '@' . request()->getHost() }}"><small>+
                                {{ trans('core/setting::setting.email_add_more') }}</small></a>
                        ...
                        <div class="ui-select-wrapper">
                            <select name="time_zone" class="ui-select form-control select-search-full" id="time_zone">
                                @foreach (DateTimeZone::listIdentifiers() as $timezone)
                                    <option value="{{ $timezone }}" @if (setting('time_zone', 'UTC') === $timezone) selected @endif>
                                        {{ $timezone }}</option>
                                @endforeach
                            </select>
                            ...
                            <select name="locale" class="ui-select form-control select-search-full" id="locale">
                                ...
                                @foreach (\Botble\Base\Supports\Language::getAvailableLocales() as $key => $locale)
                                    <option value="{{ $locale['locale'] }}"
                                        @if ($defaultLocale === $locale['locale']) selected @endif>{{ $locale['name'] }} -
                    ...
                    <div class="form-group mb-3">
                        ...
                        <label>
                            <input type="checkbox" value="1" @if (setting('enable_send_error_reporting_via_email')) checked @endif
                    ...
                    @if (count(Assets::getThemes()) > 1)
                        <div class="form-group mb-3">
                            ...
                            <label><input type="checkbox" value="1"
                                    @if (setting('enable_change_admin_theme')) checked @endif name="enable_change_admin_theme">
                                {{ trans('core/setting::setting.general.enable_change_admin_theme') }} </label>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <label class="me-2">
                            <input type="radio" name="enable_cache" value="1"
                                @if (setting('enable_cache')) checked @endif>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <label class="me-2">
                            <input type="radio" name="cache_admin_menu_enable" value="1"
                                @if (setting('cache_admin_menu_enable')) checked @endif>
                            {{ trans('core/setting::setting.general.yes') }}
                        </label>
                        <label>
        ...
        <div class="flexbox-annotated-section">
            ...
            <div class="flexbox-annotated-section-content">
                <div class="wrapper-content pd-all-20">
                    <div class="form-group mb-3">
                        <label class="text-title-field" for="buyer_commission">Buyer commission *</label>
                        <input type="text" class="next-input" name="buyer_commission" id="buyer_commission"
                            value="{{ setting('buyer_commission', '0%') }}">
                    </div>
                    <div class="form-group mb-3">
                        <label class="text-title-field" for="seller_commission">Seller commission *</label>
                        <input type="text" class="next-input" name="seller_commission" id="seller_commission"
                            value="{{ setting('seller_commission', '0%') }}">
                    </div>
