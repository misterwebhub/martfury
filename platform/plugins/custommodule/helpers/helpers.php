...
if (!function_exists('get_custommodule_page_id')) {
    /**
     * @return int
     */
    function get_custommodule_page_id(): int
    {
        return (int)theme_option('custommodule_page_id', setting('custommodule_page_id'));
    }
}

if (!function_exists('get_custommodule_page_url')) {
    /**
     * @return string
     */
    function get_custommodule_page_url(): string
    {
        $custommodulePageId = (int)theme_option('custommodule_page_id', setting('custommodule_page_id'));

        if (!$custommodulePageId) {
            return url('/');
        }

        $custommodulePage = app(\Botble\Page\Repositories\Interfaces\PageInterface::class)->findById($custommodulePageId);

        if (!$custommodulePage) {
            return url('/');
        }

        return $custommodulePage->url;
    }
}
