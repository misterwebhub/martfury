...
use Botble\CustomModule\Services\BlogService;
...
class HookServiceProvider extends ServiceProvider
{
    public function boot()
    {
            ...
            if (function_exists('admin_bar')) {
                View::composer('*', function () {
                    if (Auth::check() && Auth::user()->hasPermission('posts.create')) {
                        admin_bar()->registerLink(trans('plugins/CustomModule::posts.post'), route('posts.create'), 'add-new');
                    }
                });
            }
        });
        ...
        if (function_exists('add_shortcode')) {
            add_shortcode(
                'CustomModule-posts',
                trans('plugins/CustomModule::base.short_code_name'),
                trans('plugins/CustomModule::base.short_code_description'),
                [$this, 'renderBlogPosts']
            );
            shortcode()->setAdminConfig('CustomModule-posts', function ($attributes, $content) {
                return view('plugins/CustomModule::partials.posts-short-code-admin-config', compact('attributes', 'content'))
                    ->render();
            });
        }
    ...
    public function addThemeOptions()
    {
        ...
        theme_option()
            ->setSection([
                'title'      => 'CustomModule',
                'desc'       => 'Theme options for CustomModule',
                'id'         => 'opt-text-subsection-CustomModule',
                'subsection' => true,
                'icon'       => 'fa fa-edit',
                'fields'     => [
                    [
                        'id'         => 'custommodule_page_id',
                        'type'       => 'customSelect',
                        'label'      => trans('plugins/CustomModule::base.custommodule_page_id'),
                        'attributes' => [
                            'name'    => 'custommodule_page_id',
                            'list'    => ['' => trans('plugins/CustomModule::base.select')] + $pages,
                            'value'   => '',
                            'options' => [
                                'class' => 'form-control',
                            ],
                    ...
                    [
                        ...
                        'type'       => 'number',
                        'label'      => trans('plugins/CustomModule::base.number_posts_per_page_in_category'),
                        'attributes' => [
                    ...
                    [
                        ...
                        'type'       => 'number',
                        'label'      => trans('plugins/CustomModule::base.number_posts_per_page_in_tag'),
                        'attributes' => [
    ...
    public function registerMenuOptions()
    {
        if (Auth::user()->hasPermission('categories.index')) {
            Menu::registerMenuOptions(Category::class, trans('plugins/CustomModule::categories.menu'));
        }

        if (Auth::user()->hasPermission('tags.index')) {
            Menu::registerMenuOptions(Tag::class, trans('plugins/CustomModule::tags.menu'));
        }
    }
    ...
    public function registerDashboardWidgets($widgets, $widgetSettings)
    {
        ...
        Assets::addScriptsDirectly(['/vendor/core/plugins/CustomModule/js/CustomModule.js']);

        return (new DashboardWidgetInstance())
            ->setPermission('posts.index')
            ->setKey('widget_posts_recent')
            ->setTitle(trans('plugins/CustomModule::posts.widget_posts_recent'))
            ->setIcon('fas fa-edit')
            ->setColor('#f3c200')
    ...
    public function renderBlogPosts($shortcode)
    {
        ...
        $view = 'plugins/CustomModule::themes.templates.posts';
    ...
    {
        if ($page->id == theme_option('custommodule_page_id', setting('custommodule_page_id'))) {
            $view = 'plugins/CustomModule::themes.loop';
    ...
    {
        if ($page->id == theme_option('custommodule_page_id', setting('custommodule_page_id'))) {
            $subTitle = Html::tag('span', trans('plugins/CustomModule::base.custommodule_page'), ['class' => 'additional-page-name'])
                ->toHtml();
