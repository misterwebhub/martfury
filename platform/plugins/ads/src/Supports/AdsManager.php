...
class AdsManager
{
    ...
    /**
     * @param array $ads_info
     * 
     */
    public function display_vendor_ads(array $ads_info = [])
    {
        $html = '';
        $image = Html::image(RvMedia::getImageUrl($ads_info['image']), $ads_info['name'], ['style' => 'max-width: 100%'])
            ->toHtml();

        if ($ads_info['link'] != "#" && $ads_info['link'] != '') {
            $image = Html::link(route('public.ads-click', $ads_info['link']), $image, ['target' => '_blank'], null, false)
                ->toHtml();
        }

        $html = Html::tag('div', $image, [])->toHtml();
        return $html;
    }
